...
# 	Author: 	Dex
# 	Date:		2018/03/30
# 	Purpose:	This script is used to ping devices on the wifi network and send alerts when they either leave or arrive

base_ip=192.168.1.0
root_dir=${WIFI_PRESENCE_ALERTER_ROOT}
...
check_device_in_state(){
	...
	state_from_file=`grep "${person}|${ip_address}" ${state_file}`
	exists=$?
	if [[ ${exists} == 1 ]]
	then
		echo "${person}|${ip_address}=${ping_result}" >> ${state_file}
	else
		new_state=`echo ${state_from_file} | cut -d"=" -f2`
		compare_states ${ping_result} ${new_state} ${ip_address} ${person}
	fi
}
...
process_device_file(){
	...
	while read -r device || [[ -n "$device" ]]; do
        	...
        	echo "Checking [${person}]'s device: ${ip_address}"
        	ping_address ${ip_address}
        	ping_result=${?}

        	check_device_in_state ${ip_address} ${ping_result} ${person}
	done < ${device_list}
